...
//Home Page, start
    products: async (req, res, next) => {
        try{
            /*Check apakah users masih login
            if (data?.status) return res.status(data.status).json(data);
            let destinasi = 'SELECT * FROM destinasi_db';*/
            const [destinasi] = await DB.execute('SELECT * FROM destinasi_db');
    ...
    productsdetail: async(req, res, next) => {
        try{
            ...
            dbConnection.start.query('SELECT * FROM `destinasi_db` WHERE `nama_wisata`=? AND `id`=?', [nama_wisata][id], (error, result) => {
                if (error) {
                    console.error(error);
                ...
                if (result.length === 0) {
                    ...
                    let id_wisata = result[0].id;
                    console.log(id_wisata);
                    dbConnection.start.query('SELECT * FROM `destinasi_db` WHERE `id_wisata` LIMIT 1')
                }        
            }); 
        } catch (error) {
            // Tangani kesalahan
    ...
    orders: async (req, res, next) => {
        try{
            const [perhitungan] = await DB.execute();
        } catch (error) {
            console.error(error);
            res.status(500). json({ error: 'Internal Server Error' });
        }
